...
<i id="homeBtn" class="fa-brands fa-fort-awesome"></i>
    ...
    console.log(`select * from movieticket where tel='010-${req.query.number}' or number='${req.query.number}'`);
			db_handle.query(`select * from movieticket where tel='010-${req.query.number}' or number='${req.query.number}'`, function (err, rows) {
				if (err) {
					throw err;
				} else if (rows[0]) {
					//에러가 안났으면
                  ...
                  <section class="main__body">
                    <section class="reCeiptBody">
                      ...
                      <figure class="reCeipinfo">
                        ...
                        <figcaption class="totalAmount rcifo">
                          <h3 id="totalPriceText" class="rcifotitle">총 ${Number(
														rows[0]['price']
													).toLocaleString()}원</h3>
                        </figcaption>
                      </figure>
